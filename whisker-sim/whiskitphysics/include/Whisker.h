...
private:
  ...
  btScalar calc_mass(btScalar length, btScalar R, btScalar r,
                     btScalar rho) const;
  ...
  btScalar calc_damping(btScalar k, btScalar mass, btScalar CoM, btScalar zeta,
                        btScalar dt) const;
...
public:
  Whisker(btDiscreteDynamicsWorld *world, GUIHelperInterface *m_guiHelper,
          btAlignedObjectArray<btCollisionShape *> *shapes, std::string w_name,
          Parameters *parameters);
  ~Whisker() {}

  int idx;
  void buildWhisker(btRigidBody *refBody, btTransform offset);
  void whisk(btScalar a_vel_0, btScalar a_vel_1, btScalar a_vel_2,
