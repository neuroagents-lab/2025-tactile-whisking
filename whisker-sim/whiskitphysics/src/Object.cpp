...
Object::Object(GUIHelperInterface *helper, btDiscreteDynamicsWorld *world,
               btAlignedObjectArray<btCollisionShape *> *shapes,
               btTransform trans, std::string filename, btVector4 color,
               float scaling, float mass, int colGroup, int colMask, bool useConvexHull) {
  ...
  if (filename.compare("") != 0) {
    if (mass == 0.) {
      body = obj2StaticBody(filename, color, obj_trans, obj_orient, mass,
                            scaling, helper, shapes, world);
    } else {
      body = obj2DynamicBody(filename, color, obj_trans, obj_orient, mass,
                             scaling, useConvexHull, helper, shapes, world);
    }
...
btRigidBody *Object::obj2DynamicBody(
    std::string fileName, btVector4 color, btVector3 position,
    btQuaternion orientation, btScalar mass, float scaling_factor,
    bool useConvexHull,
  ...
  hull = new btConvexHullShape((const btScalar *)(&(v.xyzw[0])),
  ...
  btCollisionShape *shape_compound = LoadShapeFromObj(
      fileName.c_str(), "", btVector3(scaling[0], scaling[1], scaling[2]), useConvexHull);
  shape_compound->setMargin(0.5);
  m_collisionShapes->push_back(shape_compound);
  btRigidBody *body =
      createDynamicBody(mass, 0.5, trans, shape_compound, m_guiHelper, color);
  ...
  int renderInstance = m_guiHelper->registerGraphicsInstance(
      shapeId, bodyTransform.getOrigin(), bodyTransform.getRotation(), color,
      scaling);
...
// function to add objects (point clouds) as rigid bodies
btRigidBody *Object::obj2StaticBody(
    std::string fileName, btVector4 color, btVector3 position,
    btQuaternion orientation, btScalar mass, float scaling_factor,
    GUIHelperInterface *m_guiHelper,
    btAlignedObjectArray<btCollisionShape *> *m_collisionShapes,
  ...
  for (int i = 0; i < num_point / 3; i++)
    meshInterface->addTriangle(hull->getScaledPoint(i * 3),
                               hull->getScaledPoint(i * 3 + 1),
                               hull->getScaledPoint(i * 3 + 2));
  ...
  btRigidBody *body =
      createDynamicBody(0, 0.5, trans, trimesh, m_guiHelper, color);
