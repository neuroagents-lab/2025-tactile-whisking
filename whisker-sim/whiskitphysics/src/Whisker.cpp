...
Whisker::Whisker(btDiscreteDynamicsWorld *world, GUIHelperInterface *helper,
                 btAlignedObjectArray<btCollisionShape *> *shapes,
                 std::string w_name, Parameters *parameters) {
  ...
  // dt ~ whisker springy factor
  // dt = 0.0001; // parameters->TIME_STEP;
  dt = 1.0/parameters->HZ;
  ...
  col = config.col;
  length = config.L; // mm
  num_links = int(parameters->NUM_LINKS_PER_MM * length);
  printf("num_links: %d\n", num_links);
  ...
  rho = parameters->RHO_BASE /
        pow(SCALE, 3); // rho: density, SCALE: convert kg/m3 to kg/mm3
  rho_slope =
      ((parameters->RHO_TIP - parameters->RHO_BASE) / length / pow(SCALE, 3));
  zeta = parameters->ZETA; // zeta: damping ratio
  E = parameters->E /
      SCALE; // E: Young's modulus, SCALE: convert kg/m/s2 to kg/mm/s2
}

void Whisker::buildWhisker(btRigidBody *head, btTransform head2origin) {
  /// CREATE BASE POINT
  /// This is a box shape that is only translated from origin to basepoint
  /// location. It's body frame is global axis-aligned.
  ...
  btCollisionShape *basepointShape =
      new btBoxShape(btVector3(radius_base, radius_base, radius_base));
  m_collisionShapes->push_back(basepointShape);
  basepoint = createDynamicBody(btScalar(100), friction, basepointTransform,
                                basepointShape, m_guiHelper, color);
  ...
  basePointConstraint =
      new btGeneric6DofConstraint(*head, *basepoint, inFrameA, inFrameB, true);
  ...
  /// WHISKER BASE
  /// This is a sphere shape that has exactly the same transform as the
  /// basepoint (box). In non-whisking mode, it's body frame is axis-aligned. In
  /// whisking mode, this node serves as a moving node that receives angular
  /// velocity parameter from Knutsen.
  ...
  btCollisionShape *baseShape = new btSphereShape(radius_base * 2.5f);
  m_collisionShapes->push_back(baseShape);
  base = createDynamicBody(btScalar(10), friction, baseTransform, baseShape,
                           m_guiHelper, color);
  ...
  motorConstraint =
      new btGeneric6DofConstraint(*basepoint, *base, inFrameA, inFrameB, true);
  ...
  for (int i = 0; i < num_links; ++i) {
    ...
    btScalar com_distal =
        calc_com((num_links - i) * link_length, radius, radius_tip); // in cm
    btScalar mass_distal =
        calc_mass((num_links - i), radius, radius_tip, rho); // in kg
    btScalar damping =
        calc_damping(stiffness, mass_distal, com_distal, zeta, dt);

    // generate shape for unit
    btTruncatedConeShape *linkShape = new btTruncatedConeShape(
        radius * BLOW, radius_next * BLOW, link_length, 0);
    ...
    if (i == 0) {
      btTransform rotTransform =
          rotZ(base_rot[0]) * rotY(base_rot[1]) * rotX(base_rot[2]);
      btTransform transTransform =
          createFrame(btVector3(link_length / 2.f, 0, 0));
      totalTransform = prevTransform * rotTransform * transTransform;
    } else {
      btTransform linkTransform1 =
          createFrame(btVector3(link_length / 2.f, 0, 0));
      btTransform linkTransform2 =
          createFrame(btVector3(0, 0, 0), btVector3(0, 0, angle));
      btTransform linkTransform3 =
          createFrame(btVector3(link_length / 2.f, 0, 0));
      totalTransform =
          prevTransform * linkTransform1 * linkTransform2 * linkTransform3;
    }

    // add unit to whisker and world
    btRigidBody *link = createDynamicBody(mass, friction, totalTransform,
                                          linkShape, m_guiHelper, color, 0);
    ...
    if (i == 0) {
      ...
      btTransform frameInPrev =
          rotZ(base_rot[0]) * rotY(base_rot[1]) * rotX(base_rot[2]);
      baseConstraint = new btGeneric6DofSpringConstraint(
          *link_prev, *link, frameInPrev, frameInCurr, true);
      ...
      btTransform frameInPrev =
          createFrame(btVector3(link_length / 2.f, 0, 0), btVector3(0, 0, 0));
      btTransform frameInCurr =
          createFrame(btVector3(-link_length / 2.f, 0, 0), btVector3(0, 0, 0));
      ...
      spring->setAngularLowerLimit(
          btVector3(0., 1., 1.)); // lock angles between links at x axis but
                                  // free around y and z axis
      spring->setAngularUpperLimit(btVector3(0., 0., 0.));

      // add constraint to world
      m_dynamicsWorld->addConstraint(
          spring, true); // true -> collision between linked bodies disabled
...
void Whisker::whisk(btScalar a_vel_0, btScalar a_vel_1, btScalar a_vel_2,
                    btVector3 headAngularVelocity) {
...
btRigidBody *Whisker::get_unit(int idx) const { return whisker[idx]; }

btRigidBody *Whisker::get_base() const { return base; }
...
float Whisker::get_dzeta(int index) const { return dzeta[index]; }

// function to get phi angle of whisker motion (depends on row)
float Whisker::get_dphi(int index) const { return dphi[index]; }
...
btScalar Whisker::calc_slope(btScalar S, btScalar rbase, int row,
                             int col) const {
  // unit: -
  btScalar slope =
      0.0012 + 0.00017 * row - 0.000066 * col + 0.00011 * col * col;
...
btScalar Whisker::calc_mass(btScalar length, btScalar R, btScalar r,
                            btScalar rho) const {
...
btScalar Whisker::calc_com(btScalar length, btScalar R, btScalar r) const {
  btScalar com = length / 4 * (pow(R, 2) + 2 * R * r + 3 * pow(r, 2)) /
                 (pow(R, 2) + R * r + pow(r, 2));
  return com;
}
...
btScalar Whisker::calc_stiffness(btScalar E, btScalar I,
...
btScalar Whisker::calc_damping(btScalar k, btScalar M, btScalar CoM,
                               btScalar zeta, btScalar dt) const {
...
whisker_config Whisker::get_config(std::string wname, Parameters *parameters) {
  ...
  // std::cout << "Loading :  " << parameters->dir_param << std::endl;
  ...
  read_csv_float(parameters->dir_param + parameters->file_whisking_init_angle,
                 whisker_bp_angles);
  ...
  for (int i = 0; i < whisker_names.size(); i++) {
    ...
    if (!wname.compare(whisker_names[i])) {
      ...
      wc.base_pos = btVector3(whisker_bp_coor[i][0], whisker_bp_coor[i][1],
                              whisker_bp_coor[i][2]); // unit: mm
      wc.base_rot =
          btVector3(whisker_bp_angles[i][0] - PI / 2, -whisker_bp_angles[i][1],
                    whisker_bp_angles[i][2] + PI / 2);
      wfound = 1;
      break;
    }
  }
