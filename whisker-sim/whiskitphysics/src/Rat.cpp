...
Rat::Rat(GUIHelperInterface *helper, btDiscreteDynamicsWorld *world,
         btAlignedObjectArray<btCollisionShape *> *shapes,
         Parameters *parameters, btTransform origin_tf) {
  // set initial position and orientation of rat head
  btVector3 position =
      btVector3(parameters->RATHEAD_LOC[0], parameters->RATHEAD_LOC[1],
                parameters->RATHEAD_LOC[2]) + origin_tf.getOrigin();
  btVector3 orientation =
      btVector3(parameters->RATHEAD_ORIENT[0], parameters->RATHEAD_ORIENT[1],
                parameters->RATHEAD_ORIENT[2]);
  ...
  btVector4 color = btVector4(0.4, 0.4, 0.4, 1);
  rathead =
      new Object(helper, world, shapes, headTransform, parameters->dir_rathead,
                 color, SCALE / (10 * 2), 0, COL_HEAD, headCollidesWith, true);
  ...
  for (int w = 0; w < parameters->WHISKER_NAMES.size(); w++) {
    Whisker *whisker = new Whisker(world, helper, shapes,
                                    parameters->WHISKER_NAMES[w], parameters);
    whisker->buildWhisker(rathead->body, head2origin);
    m_whiskerArray.push_back(whisker);
  }
}

btAlignedObjectArray<Whisker *> Rat::getArray() { return m_whiskerArray; }

Whisker *Rat::getWhisker(int index) { return m_whiskerArray[index]; }
